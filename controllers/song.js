/**
 * Created by drodriguez on 09/11/17.
 */
'use strict'
    ...
    Song.findByIdAndUpdate(songId, update, (err,songUpdated) => {
        if(err){
            ...
            if(!songUpdated){
                ...
                res.status(202).send({song: songUpdated});
            }
        }
    });
}
    ...
    Song.findByIdAndRemove(songId, (err,songRemoved)=>{
        if(err){
            ...
            if (!songRemoved) {
                ...
                res.status(202).send({song: songRemoved});
            }
        }
    });
}
...
function uploadFile(req,res){
    ...
    if(req.files){
        ...
        res.status(500).send({message: 'No has subido ninguna cancion.'});
    }
}
